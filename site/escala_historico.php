    ...
    <div id="top" class="container-fluid">
        <div class="col-md-5">
            <form action="escala_historico" method="post">
                Mês:
                <select style="border-radius:4px; padding: 4px; border-top: solid #A9A9A9 2px; border-left: solid #A9A9A9 2px; border-right: 0px; border-bottom: 0px;" id="pegmes" name="pegmes" placeholder="<?php if(!isset($_POST['pegmes']) || empty($_POST['pegmes'])){$dtm = date('m'); echo $dtm;}?>">
                    <?php 
            ...
            <table class="table table-striped table-bordered table-hover">
                <thead>
                    ...
                    <?php
                        ...
                        $resultado = $folga->exibirEscalaTurnoFuncionario($idunidade_login, $cargo_login, $turno_login, $mesinformado, $ano_informado); 
                        foreach ($resultado as $lin){ 
                            $idusuario   = $lin['idusuario'];
                            $usuarionome = $lin['usuario'];
                            $tipofolga   = $lin['tipo'];
                            $diafolga    = $lin['dia'];
                            $mesfolga    = $lin['mes'];
                            $anofolga    = $lin['ano'];
                            echo"<tr>";
                            echo"<th style='background-color:#808080;'>{$lin['idusuario']}</th><th style='background-color:#C0C0C0;'>{$lin['usuario']}</th>";
                            
                            for($i= 1; $i <= $mesDias; $i++){
                                
                                if($diafolga == $i){
                                    
                                    if((int)$mesinformado < (int)$mesAtual && (int)$ano_informado <= (int)$anoAtual){   
                                        switch ($tipofolga){
                                            case"sap":
                                        ...
                                        switch ($tipofolga){
                                            case"sap":
                                                echo '<td style="background-color:#98FB98;"><a href="editar_folga&idusuario='.$lin['idusuario'].'&idfolga='.$lin['idfolga'].'">'.$tipofolga.'</a></td>';
                                                break;
                                            case"abo":
